...
def decrypt():
    ...
    if password == "1234":
        try:
            # Creating a new window for decryption
            screen2 = Toplevel(screen)
            screen2.title("Decryption")
            screen2.geometry("400x200")
            screen2.configure(bg="#00bd56")

            message = text1.get(1.0, END).strip()  # Strip removes extra whitespace
            ...
            messagebox.showerror("Decryption Error", "Invalid encrypted message or format.")
...
def encrypt():
    ...
    if password == "1234":
        # Creating a new window for encryption
...
def main_screen():
    ...
    # Icon for the window (optional, replace with a valid path if needed)
